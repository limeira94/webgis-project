...
def run(process,file,name):
    ...
    if process=='cover':
        ...
        xml_payload = xml_content.replace('{name}', name).replace('{file}', 'http://127.0.0.1:8000/'+file).replace('{workspace}', workspace)
    elif process=='layer':
...
def get_all_layers_from_workspace(workspace):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            data = [i['href'] for i in data if i['name'].split(":")[0]==workspace]
        else:
